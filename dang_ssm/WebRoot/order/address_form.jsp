...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			/*$(function(){
				$.post('findAddress.do','userId='+id,function(data){
					var arr=data;
					for(var i=0;i<arr.length;i++){
						$('#address').append('<option value='+arr[i].id+'>'+arr[i].fullAddress+'</option>');
					}
					
				},'json');
				$('#address').change(function(){
					$.ajax({url:'findAddressById.do',data:'id='+$('#address').val(),
	...
	<body>
		...
		<form name="ctl00" method="post" action="createOrder.do" id="f">
			<p>
				选择地址：
				...
				<div class="login_in">
					...
					<a href="../order/orderInfo.do"><input id="btnClientRegister" class="button_1" name="submit"
					type="submit" value="取消" /></a>
			
				<input id="btnClientRegister" class="button_1" name="submit"
					type="submit" value="下一步"  />
					<span class="create_order_error"></span>
				</div>
			</form>
		</div>
